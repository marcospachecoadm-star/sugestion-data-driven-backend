...
const NIQ_STOCKOUT_ALERT_DAYS = Number(process.env.NIQ_STOCKOUT_ALERT_DAYS || 2);
const NIQ_STOCKOUT_WATCH_DAYS = Number(process.env.NIQ_STOCKOUT_WATCH_DAYS || DEFAULT_SAFETY_DAYS);
  ...
  await db.doc(`insights/${dashboardDocId}`).set(
    {
      ...
      total_vendas_7d: round(totalVendas7Dias),
      total_vendas_7d_formatado: formatCurrency(totalVendas7Dias),
      ...
      vendas_7d_grafico: formatDailySalesSeries(vendas7DiasGrafico),
      vendas_7d_labels: vendas7DiasGrafico.map((item) => item.label),
      vendas_7d_valores: vendas7DiasGrafico.map((item) => round(item.valor)),
