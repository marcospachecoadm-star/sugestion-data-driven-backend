  ...
  for (const item of metricsList) {
    if (item.risco === "alto") {
      alerts.push({
        ...
        subtitulo: "Alerta de ruptura",
        descricao: "Produto com risco alto de ruptura e necessidade de reposicao.",
        tipo: "ruptura",
        tipo_label: "Ruptura",
        ...
        dias_cobertura_formatado: item.diasCobertura === null ? "Sem giro" : `${round(item.diasCobertura)} dias`,
    ...
    if (item.mediaVendaDia > 0 && item.diasCobertura !== null && item.diasCobertura > item.diasCoberturaAlvo * 2) {
      alerts.push({
        ...
        empresa_id: item.empresaId || null,
        titulo: item.produtoNome,
        subtitulo: "Alerta de excesso",
        descricao: "Produto com cobertura alta e capital imobilizado em estoque.",
        ...
        dias_cobertura_formatado: `${round(item.diasCobertura)} dias`,
        venda_perdida_estimada: round(item.vendaPerdidaEstimada),
        venda_perdida_estimada_formatada: formatCurrency(item.vendaPerdidaEstimada),
        status_niq: item.statusNiq,
        acao_recomendada: item.acaoRecomendada,
